...
table(data2008sel$Labour_status, exclude = NULL)
...
#Fig1 <- ggplot(Full_20082015.long2, aes(WAVE, value, colour=variable, shape=variable))
#Fig1 + geom_line() + expand_limits(y=c(0.15,0.3)) + geom_point()
...
Model1 <- glmer(Neigh_involve ~ (1|Neigh_year) + (1|Wijk71_sc)+ (1|WAVE_sc) + Neigh_SES + Herf_div_mean+ as.factor(Turnover_mean_5), data = Full_20082015A,family = binomial(link="logit"))
summary(Model1)
...
Full_20082015A <- Full_20082015A %>% mutate(Turnover_mean_5a= ntile(Turnover_mean, 5))
#####no significant better fits
...
Gtest <- ggplot(Full_20082015A, aes(x=factor(WAVE), y=Neigh_involve_num, group=Neigh_SES_5, colour = factor(Neigh_SES_5))) 
Gtest + stat_summary(fun.y=mean, geom="line")

Gtest2 <- ggplot(Full_20082015A, aes(x=factor(WAVE), y=Neigh_involve_num, group=Herf_div_mean_5, colour = factor(Herf_div_mean_5))) 

Gtest2 + stat_summary(fun.y=mean, geom="line")

Gtest3 <- ggplot(Full_20082015A, aes(x=factor(WAVE), y=Neigh_involve_num, group=Educ_ordinal, colour =Educ_ordinal)) 

Gtest3 + stat_summary(fun.y=mean, geom="line")

Gtest4 <- ggplot(Full_20082015A, aes(x=factor(WAVE), y=Neigh_involve_num, group=Labour_status, colour =Labour_status)) 

Gtest4 + stat_summary(fun.y=mean, geom="line")

Gtest

Full_20082015A$Volunteering_num <- as.numeric(Full_20082015A$Volunteering)

Gtest_vol <- ggplot(Full_20082015A, aes(x=factor(WAVE), y=Volunteering_num, group=Neigh_SES_5, colour = factor(Neigh_SES_5))) 
Gtest_vol + stat_summary(fun.y=mean, geom="line")

Gtest_vol2 <- ggplot(Full_20082015A, aes(x=factor(WAVE), y=Volunteering_num, group=Herf_div_mean_5, colour = factor(Herf_div_mean_5))) 

Gtest_vol2 + stat_summary(fun.y=mean, geom="line")

Gtest_vol3 <- ggplot(Full_20082015A, aes(x=factor(WAVE), y=Volunteering_num, group=Educ_ordinal, colour = Educ_ordinal)) 

Gtest_vol3 + stat_summary(fun.y=mean, geom="line")

Gtest_vol4 <- ggplot(Full_20082015A, aes(x=factor(WAVE), y=Volunteering_num, group=Labour_status, colour = Labour_status)) 

Gtest_vol4 + stat_summary(fun.y=mean, geom="line")
